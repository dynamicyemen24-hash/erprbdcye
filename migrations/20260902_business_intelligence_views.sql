...
SELECT 
  ...
  SUM(CASE WHEN tl.debit > 0 THEN tl.debit ELSE 0 END) AS total_expenses,
  SUM(CASE WHEN tl.credit > 0 THEN tl.credit ELSE 0 END) AS total_revenue,
...
SELECT 
  ...
  SUM(tl.debit) AS total_debit,
  SUM(tl.credit) AS total_credit,
  SUM(CASE WHEN coa.account_type = 'REVENUE' THEN tl.credit ELSE 0 END) AS revenue_total,
  SUM(CASE WHEN coa.account_type = 'EXPENSE' THEN tl.debit ELSE 0 END) AS expense_total,
  SUM(CASE WHEN coa.account_type = 'ASSET' THEN tl.debit ELSE 0 END) AS asset_increase,
  SUM(CASE WHEN coa.account_type = 'LIABILITY' THEN tl.credit ELSE 0 END) AS liability_increase,
  SUM(CASE WHEN coa.account_type = 'REVENUE' THEN tl.credit 
            WHEN coa.account_type = 'EXPENSE' THEN -tl.debit ELSE 0 END) AS net_position,
  SUM(CASE WHEN coa.account_type = 'REVENUE' THEN tl.credit 
            WHEN coa.account_type = 'EXPENSE' THEN -tl.debit ELSE 0 END) / 
       NULLIF(SUM(CASE WHEN coa.account_type = 'REVENUE' THEN tl.credit ELSE 0 END), 0) * 100 AS profit_margin_pct
FROM transactions t
JOIN transaction_lines tl ON tl.transaction_id = t.id
...
SELECT 
  ...
  COALESCE(SUM(CASE WHEN coa.account_type IN ('ASSET', 'EXPENSE') THEN tl.debit - tl.credit
                    WHEN coa.account_type IN ('LIABILITY', 'EQUITY', 'REVENUE') THEN tl.credit - tl.debit
...
cash_flow AS (
  SELECT 
    DATE_TRUNC('month', t.transaction_date) AS month,
    SUM(CASE WHEN tl.debit > 0 AND tl.account_id IN (SELECT id FROM cash_accounts) 
             THEN tl.debit ELSE 0 END) AS cash_inflow,
    SUM(CASE WHEN tl.credit > 0 AND tl.account_id IN (SELECT id FROM cash_accounts) 
             THEN tl.credit ELSE 0 END) AS cash_outflow,
    SUM(CASE WHEN tl.debit > 0 AND tl.account_id IN (SELECT id FROM cash_accounts) 
             THEN tl.debit
             WHEN tl.credit > 0 AND tl.account_id IN (SELECT id FROM cash_accounts) 
             THEN -tl.credit
...
SELECT 
  ...
  coa.account_type,
  COUNT(DISTINCT t.id) AS transaction_count,
  SUM(tl.debit) AS total_amount,
  AVG(tl.debit) AS average_amount,
  COUNT(DISTINCT t.created_by_id) AS users_creating,
...
SELECT 
  ...
  COALESCE(SUM(tl.debit), 0) AS total_spent,
  p.budget - COALESCE(SUM(tl.debit), 0) AS budget_remaining,
  CASE WHEN p.budget > 0 THEN (COALESCE(SUM(tl.debit), 0) / p.budget * 100)::DECIMAL(10,2) ELSE 0 END AS budget_utilization_pct,
  CASE WHEN p.end_date < NOW() AND p.progress_percent < 100 THEN 'DELAYED' 
       WHEN p.progress_percent >= 100 THEN 'COMPLETED'
       WHEN p.budget > 0 AND COALESCE(SUM(tl.debit), 0) > p.budget THEN 'OVER_BUDGET'
       ELSE p.status_code END AS health_status,
  EXTRACT(DAYS FROM (COALESCE(p.end_date, NOW()) - p.start_date)) AS planned_duration_days,
  EXTRACT(DAYS FROM (NOW() - p.start_date)) AS elapsed_days,
  CASE WHEN p.end_date > p.start_date 
       THEN (EXTRACT(DAYS FROM (NOW() - p.start_date)) / 
             EXTRACT(DAYS FROM (p.end_date - p.start_date)) * 100)::DECIMAL(10,2)
...
SELECT 
  ...
  SUM(COALESCE(SUM(tl.debit), 0)) AS total_spent,
  SUM(p.budget) - SUM(COALESCE(SUM(tl.debit), 0)) AS total_remaining,
...
SELECT 
  ...
  EXTRACT(DAYS FROM (m.target_date - NOW())) AS days_until_due,
  CASE WHEN m.completed_date IS NOT NULL 
       THEN EXTRACT(DAYS FROM (m.completed_date - m.target_date))
...
SELECT 
  ...
  EXTRACT(DAYS FROM (pt.award_date - pt.closing_date)) AS award_lead_time_days
...
SELECT 
  ...
  CASE WHEN p.budget > 0 
       THEN (COALESCE(SUM(tl.debit), 0) / p.budget * 100)::DECIMAL(10,2)
       ELSE 0 END AS utilization_pct,
...
BEGIN
  IF total_investment = 0 THEN RETURN 0;
...
BEGIN
  IF p_target_number = 0 THEN RETURN 0;
...
BEGIN
  IF p_total_requirements = 0 THEN RETURN 100;
...
BEGIN
  ...
  IF total_weight = 0 THEN RETURN 0;
...
BEGIN
  ...
  WHILE current_date <= p_end_date LOOP
    RETURN QUERY SELECT 
      current_date,
      LEAST(current_date + interval_val - INTERVAL '1 day', p_end_date),
...
BEGIN
  ...
  INSERT INTO audit_logs (
    id, table_name, record_id, action, performed_by, performed_at, details
  ) VALUES (
    gen_random_uuid(), p_table_name, p_record_id, 'DELETE', 
    p_deleted_by, v_deleted_at, 
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    SET current_balance = current_balance + 
      CASE WHEN NEW.debit > 0 THEN NEW.debit 
           WHEN NEW.credit > 0 THEN -NEW.credit
           ELSE 0 END
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    UPDATE chart_of_accounts 
    SET current_balance = current_balance - 
      CASE WHEN OLD.debit > 0 THEN OLD.debit 
           WHEN OLD.credit > 0 THEN -OLD.credit
           ELSE 0 END
...
CREATE TRIGGER trg_transaction_lines_balance
  AFTER INSERT OR UPDATE OR DELETE ON transaction_lines
  FOR EACH ROW EXECUTE FUNCTION trg_update_account_balance();
...
BEGIN
  IF TG_OP IN ('INSERT', 'UPDATE', 'DELETE') THEN
    ...
    SELECT COALESCE(SUM(tl.debit), 0), p.budget
...
CREATE TRIGGER trg_project_progress
  AFTER INSERT OR UPDATE OR DELETE ON transaction_lines
  FOR EACH ROW WHEN (COALESCE(NEW.project_id, OLD.project_id) IS NOT NULL)
  EXECUTE FUNCTION trg_update_project_progress();
...
CREATE TRIGGER trg_aid_distribution_stats
  AFTER INSERT ON distribution_beneficiaries
  FOR EACH ROW EXECUTE FUNCTION trg_update_aid_stats();
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE budget_lines
    SET spent_amount = spent_amount + NEW.debit
  ...
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE budget_lines
    SET spent_amount = spent_amount - OLD.debit
...
CREATE TRIGGER trg_update_budget
  ...
  FOR EACH ROW EXECUTE FUNCTION trg_update_budget_spent();
...
BEGIN
  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_daily_beneficiary_snapshot;
  RETURN QUERY SELECT 'mv_daily_beneficiary_snapshot'::TEXT, NOW();
  
  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_monthly_financial_rollup;
  RETURN QUERY SELECT 'mv_monthly_financial_rollup'::TEXT, NOW();
  
  REFRESH MATERIALIZED VIEW CONCURRENTLY mv_project_budget_utilization;
