...
{step === 1 && (
      ...
      <div className="flex justify-between pt-4 border-t border-slate-200 dark:border-zinc-800 mt-4">
        <EnterpriseButton variant="ghost" size="sm" onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0}>
          <ChevronRight className={`w-3 h-3 ${lang === 'ar' ? 'rotate-180' : ''}`} /> {t('السابق', 'Back')}
        </EnterpriseButton>
        <EnterpriseButton variant="primary" size="sm" onClick={handleNext} disabled={loading}>
          {loading ? <Spinner size="xs" /> : step === steps.length - 1 ? <Send className="w-3 h-3" /> : <ChevronRight className={`w-3 h-3 ${lang === 'ar' ? 'rotate-180' : ''}`} />}
  ...
  return (
    <Modal open={open} onClose={onClose} title={t('تقييم عرض ' + bid.vendor_name_ar, 'Evaluate Bid — ' + (bid.vendor_name_en || bid.vendor_name_ar))} size="md">
      ...
      <div className="flex justify-end gap-2 pt-4 border-t border-slate-200 dark:border-zinc-800 mt-4">
        <EnterpriseButton variant="ghost" size="sm" onClick={onClose}>{t('إلغاء', 'Cancel')}</EnterpriseButton>
        <EnterpriseButton variant="primary" size="sm" onClick={handleSubmit} disabled={loading}>
    ...
    <Modal open={open} onClose={onClose} title={t('المزاد الإلكتروني — ' + (tender.title_ar || ''), 'Electronic Auction — ' + (tender.title_en || tender.title_ar))} size="xl">
      <div className="space-y-4">
        ...
        {!isOwner && isLive && (
          <div className="p-3 rounded-xl bg-amber-50/60 border border-amber-200 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <EnterpriseButton variant="primary" size="sm" onClick={handleBid} disabled={placing || !bidAmount}>
                {placing && <Spinner size="xs" />}
                {t('مزايدة', 'Bid')}
              </EnterpriseButton>
            </div>
          </div>
        ...
        {isOwner && (
          <div className="flex gap-2">
            {isLive && (
              <EnterpriseButton variant="accent" size="sm" onClick={handleClose}>
                <Pause className="w-3 h-3" />{t('إنهاء المزاد', 'Close Auction')}
              </EnterpriseButton>
            )}
            {auction?.status === 'COMPLETED' && (
              <EnterpriseButton variant="primary" size="sm" onClick={() => handleAward(auction.bidder_ranks?.[0]?.vendor_id || '')}>
      ...
      <div className="space-y-4">
        ...
        {activeView === 'auction' && (
          <div className="space-y-3">
            ...
            <EnterpriseButton variant="primary" size="sm" onClick={handleAuctionLaunch}>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 shadow-xs">
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <EnterpriseButton variant="primary" size="sm" onClick={() => setCreateModalOpen(true)}>
